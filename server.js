...
function cleanJob(id) {
  const job = jobs[id];
  if (!job) return;
  if (job.rawFile) fs.unlink(job.rawFile, () => {});
...
// Auto-clean jobs older than 20 minutes
setInterval(() => {
  const now = Date.now();
  Object.keys(jobs).forEach(id => {
    if (now - Number(id) > 20 * 60 * 1000) cleanJob(id);
  });
}, 2 * 60 * 1000);
...
app.post("/prepare", (req, res) => {
  ...
  // Prefer H264/AVC streams: they are 5–10× smaller than VP9 (100–400 MB vs 2–3 GB)
  // so download is much faster. Fall back to VP9 only if H264 not available.
  const formatStr = [
  ...
  const dlArgs = ytdlpArgs([
    ...
    "--newline",      // force progress on new lines even when piped (non-TTY)
    "--progress",     // always show progress even in non-TTY
    "-o", rawFile,
    url
  ]);
  ...
  dl.stderr.on("data", parseDlChunk); // catch both just in case
  ...
  dl.on("close", code => {
    ...
    ff.stderr.on("data", chunk => {
      ...
      if (tm && totalSec > 0) {
        ...
        const pct = Math.min(95, 10 + Math.round((doneSec / totalSec) * 85));
    ...
    ff.on("close", code2 => {
      ...
      if (jobs[id]) Object.assign(jobs[id], { status: "ready", progress: 100, message: `Pronto — ${size} MB`, eta: null });
    });
...
app.get("/status", (req, res) => {
  ...
  res.json({ status: job.status, message: job.message, progress: job.progress || 0, eta: job.eta });
});
...
app.get("/file", (req, res) => {
  ...
  res.setHeader("Content-Type", job.ext === "mov" ? "video/quicktime" : "video/mp4");
  ...
  const stream = fs.createReadStream(job.outFile, { highWaterMark: 4 * 1024 * 1024 });
  stream.pipe(res);
  stream.on("close", () => setTimeout(() => cleanJob(id), 30000));
});
